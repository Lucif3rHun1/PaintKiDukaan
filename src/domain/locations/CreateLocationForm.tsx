...
export function CreateLocationForm({ onSaved, onCancel }: Props) {
  ...
  async function submit(e: SyntheticEvent<HTMLFormElement>) { e.preventDefault(); setError(null); const trimmed = name.trim(); if (!trimmed) return setError("Name is required"); setBusy(true); try { onSaved(await createLocation({ name: trimmed, zone: zone.trim() || null })); } catch (err) { setError(extractError(err)); } finally { setBusy(false); } }
