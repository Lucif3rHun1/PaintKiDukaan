...
export const createSale = (req: NewSale): Promise<number> =>
  isTauri() ? invoke<number>("cmd_create_sale", { req }) : Promise.resolve(0);
export const convertQuotation = (req: ConvertQuotation): Promise<number> =>
  isTauri() ? invoke<number>("cmd_convert_quotation", { req }) : Promise.resolve(0);
export const getSale = (id: number): Promise<Sale | null> =>
  isTauri() ? invoke<Sale | null>("cmd_get_sale", { id }) : Promise.resolve(null);
export const listSales = (fromDate?: string, toDate?: string, limit = 100): Promise<Sale[]> =>
  isTauri()
    ? invoke<Sale[]>("cmd_list_sales", { fromDate, toDate, limit })
    : Promise.resolve([]);
export const holdBill = (hb: { payload_json: string; note?: string | null }): Promise<number> =>
  isTauri() ? invoke<number>("cmd_hold_bill", { hb }) : Promise.resolve(0);
export const listHeld = (): Promise<HeldBill[]> =>
  isTauri() ? invoke<HeldBill[]>("cmd_list_held") : Promise.resolve([]);
export const deleteHeld = (id: number): Promise<boolean> =>
  isTauri() ? invoke<boolean>("cmd_delete_held", { id }) : Promise.resolve(false);

// ----- Inward / purchases -----
export const createInward = (req: NewPurchase): Promise<PurchaseCreated> =>
  isTauri() ? invoke<PurchaseCreated>("cmd_create_inward", { req }) : Promise.resolve({ id: 0, print_label: false });
export const lastCost = (itemId: number): Promise<number | null> =>
  isTauri() ? invoke<number | null>("cmd_last_cost", { itemId }) : Promise.resolve(null);
export const listPurchases = (fromDate?: string, toDate?: string, limit = 100): Promise<Purchase[]> =>
  isTauri()
    ? invoke<Purchase[]>("cmd_list_purchases", { fromDate, toDate, limit })
    : Promise.resolve([]);
export const getPurchase = (id: number): Promise<Purchase | null> =>
  isTauri() ? invoke<Purchase | null>("cmd_get_purchase", { id }) : Promise.resolve(null);
export const movementsForItem = (itemId: number, limit = 200): Promise<StockMovement[]> =>
  isTauri()
    ? invoke<StockMovement[]>("cmd_movements_for_item", { itemId, limit })
    : Promise.resolve([]);

// ----- Day close -----
export const cashSalesFor = (userId: number, date: string): Promise<CashSalesSummary> =>
  isTauri()
    ? invoke<CashSalesSummary>("cmd_cash_sales_for", { userId, date })
...
export const lastOpeningFor = (userId: number, date: string): Promise<number> =>
  isTauri()
    ? invoke<number>("cmd_last_opening_for", { userId, date })
    : Promise.resolve(0);
export const backupGateCheck = (): Promise<BackupGate> =>
  isTauri() ? invoke<BackupGate>("cmd_backup_gate_check", {}) : Promise.resolve({ needs_prompt: false, age_hours: null, reason: "browser", last_backup_at: null });
export const triggerDayClose = (req: NewDayClose): Promise<number> =>
  isTauri() ? invoke<number>("cmd_trigger_day_close", { req }) : Promise.resolve(0);
export const lockState = (userId: number, date: string): Promise<DayLockState> =>
  isTauri()
    ? invoke<DayLockState>("cmd_lock_state", { userId, date })
    : Promise.resolve({ date, user_id: userId, is_locked: false, day_close_id: null });
export const listDayClose = (limit = 60): Promise<DayClose[]> =>
  isTauri() ? invoke<DayClose[]>("cmd_list_day_close", { limit }) : Promise.resolve([]);
export const getDayClose = (id: number): Promise<DayClose | null> =>
  isTauri() ? invoke<DayClose | null>("cmd_get_day_close", { id }) : Promise.resolve(null);
export const adminReopenDay = (id: number): Promise<boolean> =>
  isTauri() ? invoke<boolean>("cmd_admin_reopen_day", { id }) : Promise.resolve(false);

// ----- Reports -----
export const dailySales = (fromDate: string, toDate: string): Promise<DailySalesReport> =>
  isTauri()
    ? invoke<DailySalesReport>("cmd_daily_sales", { fromDate, toDate })
